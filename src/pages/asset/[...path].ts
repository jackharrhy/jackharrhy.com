...
export const GET: APIRoute = async ({ params }) => {
  ...
  try {
    const vaultAssetsPath = await fs.realpath(
      path.resolve(process.env.GARDEN_VAULT_ASSETS_PATH || "./vault/Assets"),
    );
    const assetPath = await fs.realpath(
      path.resolve(vaultAssetsPath, requestedPath),
    );

    if (
      !assetPath.startsWith(`${vaultAssetsPath}${path.sep}`) &&
      assetPath !== vaultAssetsPath
    ) {
