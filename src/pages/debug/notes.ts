...
function normalizePagePath(value: unknown) {
  const pagePath = typeof value === 'string' && value.trim() ? value.trim() : '/';
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const body = await request.json().catch(() => null) as { path?: unknown; note?: unknown } | null;
  ...
  return Response.json({ path: pagePath, entry: notes.notes[pagePath] ?? null });
};
