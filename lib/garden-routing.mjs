...
const assetExtensions = new Set([
  '.avif', '.bmp', '.gif', '.jpeg', '.jpg', '.m4a', '.mov', '.mp3', '.mp4',
...
export function buildGardenIndex(entries, options = {}) {
  ...
  for (const input of entries) {
    ...
    if (existingRoute && existingRoute.sourcePath !== sourcePath) {
      index.collisions.push({ key: routeId, entries: [existingRoute, entry], type: 'route' });
    } else {
...
export function resolveWikiLink(raw, index) {
  ...
  const matches = uniqueEntries(index.lookup.get(normalizeLookupKey(parsed.targetText)) ?? []);
  const candidates = parsed.embed ? matches : matches.filter((entry) => entry.kind === 'note');
...
function normalizeLookupKey(value) {
  return stripExtension(stripGardenPrefix(normalizeVaultPath(value))).toLowerCase();
}

function stripGardenPrefix(value) {
  const normalized = normalizeVaultPath(value);
  return normalized.startsWith('Garden/') ? normalized.slice('Garden/'.length) : normalized;
}
...
function uniqueEntries(entries) {
  return [...new Map(entries.map((entry) => [entry.sourcePath, entry])).values()];
}
...
function readAliases(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inline) {
      aliases.push(...inline[1].split(',').map((alias) => alias.trim().replace(/^['"]|['"]$/g, '')).filter(Boolean));
      continue;
    }
