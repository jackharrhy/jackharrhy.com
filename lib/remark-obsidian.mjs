...
function parseObsidianDimensions(value) {
  if (!value) return {};
...
export function remarkObsidian(options = {}) {
  ...
  return (tree, file) => {
    ...
    if (src.includes("\t")) {
      ...
      visit(tree, "paragraph", (node, index, parent) => {
        ...
        for (const seg of segments) {
          ...
          while (
            seg.children.length &&
            seg.children[0].type === "text" &&
            !seg.children[0].value.trim()
          )
            seg.children.shift();
          while (
            seg.children.length &&
            seg.children.at(-1).type === "text" &&
            !seg.children.at(-1).value.trim()
          )
            seg.children.pop();
          ...
          if (seg.tabbed) {
            replacements.push({
              ...
              data: {
                hName: "div",
                hProperties: { className: ["ml-8", "mb-4", "last:mb-0"] },
              },
    ...
    // Pass 2: YouTube image embeds → responsive iframes.
    ...
    visit(tree, "text", (node, index, parent) => {
      ...
      while ((match = obsidianRegex.exec(text)) !== null) {
        ...
        if (exclaim === "!") {
          ...
          const { width, height } = parseObsidianDimensions(extra);
